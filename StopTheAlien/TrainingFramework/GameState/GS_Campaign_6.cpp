...
void GSCampaign6::Init()
{
	auto background = SceneManager::GetInstance()->GetObjectByID("map6");
...
void GSCampaign6::GunUpdate(float deltaTime)
{
	if (isBulletOut) {
...
void GSCampaign6::AlienUpdate(float deltaTime)
{
...
void GSCampaign6::BulletUpdate(float deltaTime)
{
...
void GSCampaign6::Update(float deltaTime)
{
...
void GSCampaign6::HandleEvents()
{
}

void GSCampaign6::HandleKeyEvents(int key, bool bIsPressed)
{
...
void GSCampaign6::HandleTouchEvents(float x, float y, bool bIsPressed)
{
...
void GSCampaign6::HandleMouseMoveEvents(float x, float y)
{
	for (auto& button : m_buttonList)
	{
...
void GSCampaign6::Spawn(const char* type)
{
...
void GSCampaign6::SpawnByDifficult(float deltaTime) {
	m_time -= deltaTime;
	if (m_time <= 0) {
		m_time += 1;
	...
	if (alienSpawned > 25 && m_alien.empty())
	{
		for (int i = 2; i <= 8; i++)	Mix_HaltChannel(i);
